...
void TraverseIteratorCount2(::openmldb::common::StorageMode storageMode) {
...
TEST_F(TableTest, TraverseIteratorCount2Mem) {
    TraverseIteratorCount2(::openmldb::common::StorageMode::kMemory);
}

TEST_F(TableTest, TraverseIteratorCount2Disk) {
    TraverseIteratorCount2(::openmldb::common::StorageMode::kHDD);
}

// TEST_F (TableTest, TableIteratorTS2Disk) {
//     ::openmldb::api::TableMeta table_meta;
//     table_meta.set_name("table1");
//     table_meta.set_tid(25);
//     table_meta.set_pid(1);
//     table_meta.set_seg_cnt(8);
//     table_meta.set_mode(::openmldb::api::TableMode::kTableLeader);
//     table_meta.set_key_entry_max_height(8);
...
//         table->Put(0, value, request.dimensions());
//     }
//     TableIterator* it = table->NewTraverseIterator(2);
//     it->SeekToFirst();
//     int count = 0;
//     while (it->Valid()) {
...
TEST_F (TableTest, TableIteratorTS2Disk) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        ASSERT_EQ(0, codec.EncodeRow(row, &value));
        table->Put(0, value, request.dimensions());
        table->Put(1, value, request.dimensions());

    }
    Ticket ticket;
    TableIterator* iter = table->NewIterator(0, "card10", ticket);
    iter->SeekToFirst();
    int count = 0;
    while (iter->Valid()) {
        count++;
...
//     std::map<std::string, uint32_t> mapping;
//     mapping.insert(std::make_pair("idx0", 0));
//     Table* table = Table::CreateTable("tx_log", 25, 1, 8, mapping, 0, ::openmldb::type::kAbsoluteTime,
//         FLAGS_hdd_root_path, ::openmldb::common::StorageMode::kHDD);
//     table->Init();
...
// TEST_F (TableTest, TableIteratorTS3Disk) {
//     ::openmldb::api::TableMeta table_meta;
//     table_meta.set_name("table1");
//     table_meta.set_tid(26);
//     table_meta.set_pid(1);
//     table_meta.set_seg_cnt(8);
//     table_meta.set_mode(::openmldb::api::TableMode::kTableLeader);
//     table_meta.set_key_entry_max_height(8);
...
// TEST_F (TableTest, DiskTablePutTwice) {
//     std::map<std::string, uint32_t> mapping;
//     mapping.insert(std::make_pair("idx0", 0));
//     Table* table = Table::CreateTable("tx_log", 27, 1, 8, mapping, 0, ::openmldb::type::kAbsoluteTime,
//         FLAGS_hdd_root_path, ::openmldb::common::StorageMode::kHDD);
//     table->Init();

//     table->Put("pk", 9527, "test1", 5);
//     table->Put("pk", 9527, "test2", 5);
    
//     Ticket ticket;
//     TableIterator* it = table->NewIterator("pk", ticket);
//     it->Seek(9527);
//     ASSERT_TRUE(it->Valid());
//     ASSERT_EQ("test2", it->GetValue());
