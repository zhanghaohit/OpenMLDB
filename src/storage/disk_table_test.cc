...
TEST_F(DiskTableTest, Put) {
    ...
    DiskTable* table = new DiskTable(
        "yjtable1", 1, 1, mapping, 10, ::openmldb::api::TTLType::kAbsoluteTime,
...
TEST_F(DiskTableTest, MultiDimensionPut) {
    ...
    DiskTable* table = new DiskTable(
        "yjtable2", 2, 1, mapping, 10, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
...
TEST_F(DiskTableTest, LongPut) {
    ...
    mapping.insert(std::make_pair("idx1", 1));
    DiskTable* table = new DiskTable(
        "yjtable3", 3, 1, mapping, 10, ::openmldb::api::TTLType::kAbsoluteTime,
...
TEST_F(DiskTableTest, Delete) {
    ...
    mapping.insert(std::make_pair("idx2", 2));
    DiskTable* table = new DiskTable(
        "yjtable2", 4, 1, mapping, 10, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    for (int idx = 0; idx < 10; idx++) {
        std::string key = "test" + std::to_string(idx);
...
TEST_F(DiskTableTest, TraverseIterator) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 5, 1, mapping, 0, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    for (int idx = 0; idx < 100; idx++) {
        std::string key = "test" + std::to_string(idx);
...
TEST_F(DiskTableTest, TraverseIteratorCount) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 6, 1, mapping, 0, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    for (int idx = 0; idx < 100; idx++) {
        std::string key = "test" + std::to_string(idx);
...
TEST_F(DiskTableTest, TraverseIteratorCountTTL) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 7, 1, mapping, 5, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    const auto& ttl = table->GetTTL();
    ASSERT_EQ((int64_t)ttl.abs_ttl, 5 * 60 * 1000);
    ASSERT_EQ((int64_t)ttl.lat_ttl, 0);
    ASSERT_EQ(ttl.ttl_type, ::openmldb::storage::TTLType::kAbsoluteTime);
...
TEST_F(DiskTableTest, TraverseIteratorLatest) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 8, 1, mapping, 3, ::openmldb::api::TTLType::kLatestTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    const auto& ttl = table->GetTTL();
    ASSERT_EQ((int64_t)ttl.abs_ttl, 0);
    ASSERT_EQ((int64_t)ttl.lat_ttl, 3);
    ASSERT_EQ(ttl.ttl_type, ::openmldb::storage::TTLType::kLatestTime);
    for (int idx = 0; idx < 100; idx++) {
        std::string key = "test" + std::to_string(idx);
        uint64_t ts = 9537;
        for (int k = 0; k < 5; k++) {
...
TEST_F(DiskTableTest, Load) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 9, 1, mapping, 10, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    for (int idx = 0; idx < 100; idx++) {
        std::string key = "test" + std::to_string(idx);
...
TEST_F(DiskTableTest, CompactFilter) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 10, 1, mapping, 10, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    uint64_t cur_time = ::baidu::common::timer::get_micros() / 1000;
    for (int idx = 0; idx < 100; idx++) {
...
TEST_F(DiskTableTest, GcHeadMulTs) {
    ...
    table_meta.set_pid(1);
    table_meta.set_ttl(10);
    table_meta.set_ttl_type(::openmldb::api::TTLType::kLatestTime);
    table_meta.set_storage_mode(::openmldb::common::kHDD);
    ::openmldb::common::ColumnDesc* column_desc = table_meta.add_column_desc();
    column_desc->set_name("card");
    column_desc->set_type("string");
...
TEST_F(DiskTableTest, GcHead) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 13, 1, mapping, 3, ::openmldb::api::TTLType::kLatestTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    for (int idx = 0; idx < 100; idx++) {
        std::string key = "test" + std::to_string(idx);
...
TEST_F(DiskTableTest, CheckPoint) {
    ...
    mapping.insert(std::make_pair("idx0", 0));
    DiskTable* table = new DiskTable(
        "t1", 15, 1, mapping, 0, ::openmldb::api::TTLType::kAbsoluteTime,
        ::openmldb::common::StorageMode::kHDD, FLAGS_hdd_root_path);
    ASSERT_TRUE(table->Init());
    for (int idx = 0; idx < 100; idx++) {
        std::string key = "test" + std::to_string(idx);
