...
bool ArithmeticIRBuilder::BuildAnd(::llvm::BasicBlock* block, ::llvm::Value* left, ::llvm::Value* right,
    ...
    if (!InferAndCastIntegerTypes(block, left, right, &casted_left, &casted_right, status)) {
        return false;
    }
...
bool ArithmeticIRBuilder::BuildOr(::llvm::BasicBlock* block, llvm::Value* left, llvm::Value* right,
                                  llvm::Value** output, base::Status* status) {
    if (!left->getType()->isIntegerTy() || !right->getType()->isIntegerTy()) {
        status->msg = "fail to codegen arithmetic and expr: value types are invalid";
        status->code = common::kCodegenError;
...
bool ArithmeticIRBuilder::BuildXor(::llvm::BasicBlock* block, llvm::Value* left, llvm::Value* right,
                                   llvm::Value** output, base::Status* status) {
    if (!left->getType()->isIntegerTy() || !right->getType()->isIntegerTy()) {
        status->msg = "fail to codegen arithmetic and expr: value types are invalid";
        status->code = common::kCodegenError;
...
bool ArithmeticIRBuilder::BuildNot(::llvm::BasicBlock* block, llvm::Value* input, llvm::Value** output,
                                   base::Status* status) {
    ::llvm::IRBuilder<> builder(block);
...
Status ArithmeticIRBuilder::BuildBitwiseAndExpr(const NativeValue& left, const NativeValue& right,
                                                NativeValue* output) {
    CHECK_STATUS(
        TypeIRBuilder::BinaryOpTypeInfer(node::ExprNode::BitwiseLogicalTypeAccept, left.GetType(), right.GetType()));
    CHECK_STATUS(NullIRBuilder::SafeNullBinaryExpr(
        block_, left, right,
        [](::llvm::BasicBlock* block, ::llvm::Value* lhs, ::llvm::Value* rhs, ::llvm::Value** output, Status& status) {
