...
static Status CreateNewProject(PhysicalPlanContext* ctx, ProjectType ptype, PhysicalOpNode* input,
                               const ColumnProjects& projects, const node::ExprNode* having_condition,
                               PhysicalOpNode** out) {
    switch (ptype) {
        ...
        case ProjectType::kAggregation: {
            ...
            CHECK_STATUS(ctx->CreateOp<PhysicalAggrerationNode>(&op, input, projects, having_condition));
            *out = op;
            break;
        }
        default:
...
Status CommonColumnOptimize::ProcessProject(PhysicalPlanContext* ctx, PhysicalProjectNode* project_op,
    ...
    if (having_condition.ValidCondition()) {
        bool need_update = !ExprDependOnlyOnLeft(having_condition.condition(), new_input, nullptr);
    ...
    if (common_projects.size() == origin_projects.size() && input_state->common_op == input_op) {
        ...
        CHECK_STATUS(CreateNewProject(ctx, project_op->project_type_, input_state->common_op, common_projects,
                                      new_having_condition, &common_project_op));
    ...
    if (non_common_projects.size() == origin_projects.size() && new_input == input_op) {
        ...
        CHECK_STATUS(CreateNewProject(ctx, project_op->project_type_, new_input, non_common_projects,
                                      new_having_condition, &non_common_project_op));
...
Status CommonColumnOptimize::ProcessWindow(PhysicalPlanContext* ctx, PhysicalAggrerationNode* agg_op,
    ...
    if (agg_op->having_condition_.ValidCondition()) {
        ...
        bool need_update = !ExprDependOnlyOnLeft(having_condition, agg_request_concat, nullptr);
