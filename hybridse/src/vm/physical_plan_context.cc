...
Status PhysicalPlanContext::InitFnDef(const ColumnProjects& projects, const SchemasContext* schemas_ctx,
    ...
    for (size_t i = 0; i < projects.size(); ++i) {
        ...
        CHECK_TRUE(resolved_expr->GetOutputType() != nullptr, kPlanError,
                   "Fail to resolve expression: ", resolved_expr->GetExprString());
...
Status PhysicalPlanContext::GetSourceID(const std::string& db_name, const std::string& table_name,
    ...
    CHECK_TRUE(tbl_iter != db_table_column_id_map_[dbstr].end(), kPlanError, "Fail to find source table name ",
               table_name);
...
Status PhysicalPlanContext::GetRequestSourceID(const std::string& db_name, const std::string& table_name,
                                               const std::string& column_name, size_t* column_id) {
...
Status PhysicalPlanContext::InitializeSourceIdMappings(const std::string& db_name, const std::string& table_name) {
