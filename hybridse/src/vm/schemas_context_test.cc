...
class SchemasContextTest : public ::testing::Test {
 public:
    SchemasContextTest() : catalog(new vm::SimpleCatalog()), plan_ctx(&nm, &lib, "db", catalog, nullptr, false) {}
    node::NodeManager nm;
    udf::UdfLibrary lib;
...
void CheckColumnResolveCase(const std::string& db_name, const std::string& relation_name,
                            const std::string& column_name, PhysicalOpNode* node,
    ...
    size_t schema_idx;
    size_t col_idx;
    status = schemas_context->ResolveColumnIndexByName(db_name, relation_name, column_name, &schema_idx, &col_idx);
    if (expect.is_error) {
        LOG(INFO) << status;
    ...
    status = schemas_context->ResolveColumnID(db_name, relation_name, column_name, &column_id, &child_path_idx,
...
INSTANTIATE_TEST_SUITE_P(ResolveNameTest, SchemasContextResolveTest,
                         testing::ValuesIn(sqlcase::InitCases("/cases/plan/schemas_context/resolve_column_name.yaml")));
