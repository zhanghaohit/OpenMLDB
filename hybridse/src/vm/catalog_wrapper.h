...
class IteratorFilterWrapper : public RowIterator {
 public:
    ...
    bool Valid() const override { return iter_->Valid() && predicate_->operator()(iter_->GetValue(), parameter_); }
    void Next() override {
        iter_->Next();
...
class WindowIteratorProjectWrapper : public WindowIterator {
 public:
    WindowIteratorProjectWrapper(std::unique_ptr<WindowIterator> iter, const Row& parameter, const ProjectFun* fun)
    ...
    std::unique_ptr<RowIterator> GetValue() override {
        ...
        if (!iter) {
            ...
            return std::unique_ptr<RowIterator>(new IteratorProjectWrapper(std::move(iter), parameter_, fun_));
        }
    }
...
class WindowIteratorFilterWrapper : public WindowIterator {
 public:
    WindowIteratorFilterWrapper(std::unique_ptr<WindowIterator> iter, const Row& parameter, const PredicateFun* fun)
        : WindowIterator(), iter_(std::move(iter)), parameter_(parameter), fun_(fun) {}
...
class PartitionProjectWrapper : public PartitionHandler {
 public:
    PartitionProjectWrapper(std::shared_ptr<PartitionHandler> partition_handler, const Row& parameter,
                            const ProjectFun* fun)
        : PartitionHandler(), partition_handler_(partition_handler), parameter_(parameter), value_(), fun_(fun) {}
    ...
    std::unique_ptr<WindowIterator> GetWindowIterator() override {
        ...
        if (!iter) {
            ...
            return std::unique_ptr<WindowIterator>(new WindowIteratorProjectWrapper(std::move(iter), parameter_, fun_));
        }
    }
...
class PartitionFilterWrapper : public PartitionHandler {
 public:
    PartitionFilterWrapper(std::shared_ptr<PartitionHandler> partition_handler, const Row& parameter,
                           const PredicateFun* fun)
        : PartitionHandler(), partition_handler_(partition_handler), parameter_(parameter), fun_(fun) {}
...
class TableProjectWrapper : public TableHandler {
 public:
    TableProjectWrapper(std::shared_ptr<TableHandler> table_handler, const Row& parameter, const ProjectFun* fun)
...
class RowProjectWrapper : public RowHandler {
 public:
    RowProjectWrapper(std::shared_ptr<RowHandler> row_handler, const Row& parameter, const ProjectFun* fun)
