...
std::shared_ptr<TableHandler> PartitionProjectWrapper::GetSegment(const std::string& key) {
    ...
    if (!segment) {
        ...
        return std::shared_ptr<TableHandler>(new TableProjectWrapper(segment, parameter_, fun_));
    }
}
...
std::shared_ptr<TableHandler> PartitionFilterWrapper::GetSegment(const std::string& key) {
    ...
    if (!segment) {
        ...
        return std::shared_ptr<TableHandler>(new TableFilterWrapper(segment, parameter_, fun_));
    }
}
...
std::shared_ptr<PartitionHandler> TableProjectWrapper::GetPartition(const std::string& index_name) {
    ...
    if (!partition) {
        ...
        return std::shared_ptr<PartitionHandler>(new PartitionProjectWrapper(partition, parameter_, fun_));
    }
}
std::shared_ptr<PartitionHandler> TableFilterWrapper::GetPartition(const std::string& index_name) {
    ...
    if (!partition) {
        ...
        return std::shared_ptr<PartitionHandler>(new PartitionFilterWrapper(partition, parameter_, fun_));
    }
}
}  // namespace vm
}  // namespace hybridse
