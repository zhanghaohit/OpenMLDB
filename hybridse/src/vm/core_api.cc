...
std::string CoreAPI::ResolveSourceColumnName(hybridse::vm::PhysicalOpNode* node, hybridse::node::ColumnRefNode* expr) {
    ...
    auto status = schemas_ctx->ResolveColumnID(column_expr->GetDBName(), column_expr->GetRelationName(),
                                               column_expr->GetColumnName(), &column_id, &child_path_idx,
...
ColumnSourceInfo CoreAPI::ResolveSourceColumn(hybridse::vm::PhysicalOpNode* node, const std::string& db_name,
    ...
    status = node->schemas_ctx()->ResolveColumnID(db_name, relation_name, column_name, &result.column_id_,
                                                  &result.child_path_idx_, &result.child_column_id_,
                                                  &result.source_column_id_, &result.source_node_);
...
hybridse::codec::Row CoreAPI::RowConstProject(const RawPtrHandle fn, const Row parameter, const bool need_free) {
...
hybridse::codec::Row CoreAPI::RowProject(const RawPtrHandle fn, const hybridse::codec::Row row,
                                         const hybridse::codec::Row parameter, const bool need_free) {
    if (row.empty()) {
        return hybridse::codec::Row();
    }
    // Init current run step runtime
...
hybridse::codec::Row CoreAPI::WindowProject(const RawPtrHandle fn, const uint64_t row_key, const Row row,
    ...
    uint32_t ret = udf(static_cast<int64_t>(row_key), row_ptr, window_ptr, nullptr, &out_buf);
...
hybridse::codec::Row CoreAPI::WindowProject(const RawPtrHandle fn, const uint64_t key, const Row row,
    ...
    return Runner::WindowProject(fn, key, row, Row(), is_instance, append_slices, window->GetWindow());
}
...
bool CoreAPI::ComputeCondition(const hybridse::vm::RawPtrHandle fn, const Row& row, const Row& parameter,
