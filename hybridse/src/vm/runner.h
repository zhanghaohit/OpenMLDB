...
class Runner : public node::NodeBase<Runner> {
 public:
    ...
    static Row RowConstProject(const int8_t* fn, const bool need_free = false);
    ...
    static Row WindowProject(const int8_t* fn, const uint64_t key, const Row row, const Row& parameter,
    ...
    static const Row RowLastJoinTable(size_t left_slices, const Row& left_row, size_t right_slices,
                                      std::shared_ptr<TableHandler> right_table, const hybridse::codec::Row& parameter,
...
class GroupAggRunner : public Runner {
 public:
    GroupAggRunner(const int32_t id, const SchemasContext* schema, const int32_t limit_cnt, const Key& group,
                   const ConditionFilter& having_condition, const FnInfo& project)
...
class AggRunner : public Runner {
 public:
    AggRunner(const int32_t id, const SchemasContext* schema, const int32_t limit_cnt,
              const ConditionFilter& having_condition, const FnInfo& fn_info)
        : Runner(id, kRunnerAgg, schema, limit_cnt), having_condition_(having_condition.fn_info()), agg_gen_(fn_info) {}
    ~AggRunner() {}
    std::shared_ptr<DataHandler> Run(RunnerContext& ctx,                                                 // NOLINT
                                     const std::vector<std::shared_ptr<DataHandler>>& inputs) override;  // NOLINT
    ConditionGenerator having_condition_;
...
class WindowAggRunner : public Runner {
 public:
    ...
    std::shared_ptr<DataHandler> Run(RunnerContext& ctx,                                                 // NOLINT
                                     const std::vector<std::shared_ptr<DataHandler>>& inputs) override;  // NOLINT
    void RunWindowAggOnKey(const Row& parameter, std::shared_ptr<PartitionHandler> instance_partition,
...
class ClusterJob {
 public:
    ...
    explicit ClusterJob(const std::string& sql, const std::string& db, const std::set<size_t>& common_column_indices)
        : tasks_(), main_task_id_(-1), sql_(sql), db_(db), common_column_indices_(common_column_indices) {}
...
class RunnerContext {
 public:
    explicit RunnerContext(hybridse::vm::ClusterJob* cluster_job, const hybridse::codec::Row& parameter,
